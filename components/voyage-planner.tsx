...
function toneButtonClasses(tone: BrandTone, active: boolean) {
  ...
  if (active) {
    return `${palette.button} shadow-[0_1px_0_rgba(255,255,255,0.35)_inset]`;
  }

  return palette.buttonMuted;
}
...
export function VoyagePlanner({ mode = "compact" }: VoyagePlannerProps) {
  ...
  const segments = useMemo(() => buildVoyagePlan(devices, status), [devices, status]);
    ...
    >
      ...
      {mode === "compact" ? (
        <div className="mt-5 flex items-center justify-between gap-3 border-t border-line pt-4">
          <p className="text-sm text-muted">完整航路可以继续标记完成、收藏并展开阅读。</p>
          <Link
            href="/start"
            className="inline-flex items-center gap-2 rounded-[0.85rem] border border-foreground bg-foreground px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-black"
          >
            打开完整登船指南
